...
public class Another_C extends JFrame{
	...
	public static void main(String[] args){
		JFrame frame = new Another_C();//新建框架
...
class GamePanel extends JPanel implements Runnable{
	...
	boolean isFirstRun = true;//判斷是否需要初始化
	boolean isStarted = false;//判斷是否開始
	boolean isPaused = false;//判斷是否暫停
	static int score2 = 0;//遊戲分數
	static int score = 0;//遊戲分數
	static int information = 0;//傳遞遊戲資訊
	static char dir = 'R';//遊戲分數
	
	static Snake snake = new Snake();//新建一條蛇
	static Snake snake2 = new Snake();//新建一條蛇
	static Dot dessert = new Dot();//新建一個點心
...
//			dessert.setY(yRandom);
			...
			public void keyPressed(KeyEvent e){
				...
				switch(e.getKeyCode()){
					...
					case KeyEvent.VK_ENTER:
						if(isCrushed()){//如果遊戲結束，則重置資料重新開始遊戲
	...
	public synchronized void changeSnakeLocation(){//改變蛇的位置資訊並重畫
		//存儲蛇頭先前的資訊，為蛇身的更新提供依據
	...
	public synchronized void changeSnakeLocation2(){//改變蛇的位置資訊並重畫
		//存儲蛇頭先前的資訊，為蛇身的更新提供依據
	...
	public boolean isEncountered2(){//判斷是否吃到點心
...
class InformationPanel extends JPanel implements Runnable{
	...
	JLabel score = new JLabel("分數：");//顯示分數
	JLabel score2 = new JLabel("分數：");//顯示分數
	...
	public InformationPanel(){
		...
		//配置?明資訊
		help[0].setText("Enter重製遊戲");
		help[1].setText("再按一次Enter開始");
		help[2].setText("方向鍵移動蛇");
		help[3].setText("請等待Server端開始遊戲");
		...
		b1.addActionListener(new ActionListener() {
	        public void actionPerformed(ActionEvent e) {
	        	...
	        	if(textA.getText()!="")
	        		...
	        		textA.setText("我方:"+textF.getText());
	        	textF.setText("");
	        }
	    });
	...
	public void run(){//更新遊戲資訊
		while(true){
			String string1 = "snack1分數：" + Integer.toString(GamePanel.getScore());
			score.setText(string1);
			String string12 = "snack2分數：" + Integer.toString(GamePanel.getScore2());
			score2.setText(string12);
			String string2 = null;
			switch(GamePanel.getInformation()){
				case 0:break;
				case 1:string2 = "你撞穿牆壁了！";break;
				case 2:string2 = "snack1你吃到自己了！";break;
				case 3:string2 = "snack2你吃到自己了！";break;
				case 4:string2 = "蛇互撞了!！";break;
				default:
			}
...
class connect implements Runnable{
	...
	public void run(){//更新遊戲資訊
	  ...
	  try {
	   sk = new Socket("127.0.0.1",5054);    //建立Socket物件，並設定ip(127.0.0.1是自己的主機)和埠號
	   System.out.println("已連線Server5054");
...
class connect2 implements Runnable{
	...
	public void run(){//更新遊戲資訊
	  ...
	  try {
	   sk = new Socket("127.0.0.1",5052);    //建立Socket物件，並設定ip(127.0.0.1是自己的主機)和埠號
	   System.out.println("已連線Server5052");
	   theInputStream = new BufferedReader(new InputStreamReader(sk.getInputStream()));
	   theOutputStream = new PrintStream(sk.getOutputStream());
	   while(sk.isConnected()){
...
class chat implements Runnable{
	...
	public void run(){//更新遊戲資訊
	  ...
	  try {
	   sk = new Socket("127.0.0.1",5056);    //建立Socket物件，並設定ip(127.0.0.1是自己的主機)和埠號
	   System.out.println("已連線Server5056");
	   theInputStream = new BufferedReader(new InputStreamReader(sk.getInputStream()));
	   theOutputStream = new PrintStream(sk.getOutputStream());
	   while(sk.isConnected()){
		   if(sen==1) {
...
class chat2 implements Runnable{
	...
	public void run(){//更新遊戲資訊
	  try {
		  sk = new Socket("127.0.0.1",5055);    //建立Socket物件，並設定ip(127.0.0.1是自己的主機)和埠號
		   System.out.println("已連線Server5055");
		   theInputStream = new BufferedReader(new InputStreamReader(sk.getInputStream()));
		   theOutputStream = new PrintStream(sk.getOutputStream());
	   while(sk.isConnected()){
		   msg = theInputStream.readLine();
